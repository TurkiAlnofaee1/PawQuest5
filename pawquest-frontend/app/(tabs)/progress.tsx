...
// ✅ new import
import { ensurePlayersDB } from "@/src/lib/ensurePlayersDB";
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!auth.currentUser) {
          try {
            await signInWithEmailAndPassword(
              auth,
              "2240569@uj.edu.sa",
              "2240569A_" // for testing only
            );
            console.log(
              "✅ Signed in for test:",
              (auth.currentUser as import("firebase/auth").User | null)?.email
            );
          } catch (e: any) {
            console.warn("⚠️ Test sign-in failed:", e?.message);
          }
        }
        ...
        await ensurePlayersDB();

        const { start, end, keys } = build7DayKeys(new Date());

        // range query on dayStartTS
        const qRef = query(
          collection(db, "PlayersDB", uid, "dailyStats"),
          where("dayStartTS", ">=", start),
          where("dayStartTS", "<=", end),
          orderBy("dayStartTS", "asc")
        );

        const snap = await getDocs(qRef);

        // map doc.id (YYYY-MM-DD) -> {calories, steps}
        const map = new Map<
          string,
          { calories?: number; steps?: number; dayStartTS?: Timestamp }
        >();
        snap.forEach((d) => {
        ...
        // zero-fill by keys order
        const calArr = keys.map((k) => map.get(k)?.calories ?? 0);
        const stepArr = keys.map((k) => map.get(k)?.steps ?? 0);
